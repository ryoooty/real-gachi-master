# Workout Planner Bot (aiogram + APScheduler)

Демонстрационный бот реализует требования плана:

- Хранение профиля, недельных планов и дневных логов в SQLite.
- Конвертация пользовательского времени в UTC через `zoneinfo` и планирование задач APScheduler по UTC.
- Еженедельная генерация плана (заглушка DeepSeek) с возможностью программного усиления при отклике «легко».
- Reply-меню для навигации и Inline чек-листы упражнений с динамическими галочками и оценкой сложности.

## Запуск

```bash
pip install -r requirements.txt
python -m app.bot  # В main() используется токен-заглушка, подставьте свой
```

### Windows без виртуального окружения

1. Убедитесь, что установлен Python 3.10+ (python.org или Microsoft Store).
2. В командной строке (cmd/PowerShell) перейдите в папку проекта и установите зависимости сразу в глобальную среду пользователя:
   ```powershell
   pip install --user -r requirements.txt
   ```
3. Пропишите токен бота в переменной окружения (в PowerShell):
   ```powershell
   setx BOT_TOKEN "123:ABC"
   ```
   Перезапустите терминал, чтобы переменная подтянулась.
4. Запустите бота:
   ```powershell
   python -m app.bot
   ```
5. Если увидите ошибку про часовые пояса (`zoneinfo`), установите данные зон:
   ```powershell
   pip install --user tzdata
   ```

## Основные файлы

- `app/database.py` — создание схемы и доступ к таблицам `users`, `weekly_plan`, `daily_logs`.
- `app/time_utils.py` — конвертация локального времени пользователя в UTC без ручной арифметики.
- `app/ai.py` — клиент и парсер недельного плана DeepSeek с корректировкой нагрузок.
- `app/keyboards.py` — Reply-меню и Inline-кнопки чек-листа/сложности.
- `app/scheduler.py` — обертка над APScheduler для ежедневных уведомлений по UTC.
- `app/bot.py` — FSM профиля, отправка чек-листов, обратная связь по сложности и начисление очков.

## Соответствие техническому плану

| Пункт плана | Статус | Комментарий |
| --- | --- | --- |
| База и меню (SQLite, Reply-клавиатура, FSM профиля) | ✅ | Таблицы `users`, `weekly_plan`, `daily_logs`, ограничение на двух пользователей через middleware и валидация ввода профиля. |
| Генерация плана AI + JSON | ✅ | Заглушка DeepSeek, еженедельная генерация по cron с учетом статистики/отзыва сложности и команда `/generate` для ручного запуска. |
| «Живой» чек-лист с галочками и оценкой сложности | ✅ | Галочки отображаются в тексте, пропуск дня фиксируется, после выполнения доступна оценка сложности. |
| Система очков и статистика | ✅ | Очки начисляются за выполненные упражнения, подсчитывается стрик и отображается лидерборд в меню «📈 Статистика». |
| Scheduler и Timezones | ✅ | Хранение времени в UTC, поддержка точного времени и диапазона, восстановление задач при старте и еженедельный cron на генерацию плана. |
